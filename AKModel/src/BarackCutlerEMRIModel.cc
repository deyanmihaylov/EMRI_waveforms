...
void findfrequency(double dt,double *par, double eisco,double nuisco, double *e0, double *nu0) {

  int nsteps,i,n;
  n=1000000;
  double edot,nudot;
  double t0,mu,M;
  double Z,Y,e2,timestep;
  double *e,*nu;
  e=(double *)malloc(n*sizeof(double));
  nu=(double *)malloc(n*sizeof(double));

  timestep=100;
  nsteps=int(dt/2/timestep);
  mu=par[2];
  M=par[3];
  e[0]=eisco;
  nu[0]=nuisco;

  //     <<< EVOLVE BACKWARD FROM isco to t0 >>>
  
  for (i=0;i<nsteps;i++) {
    Z=pow((2.*M_PI*M*nu[i]),1./3.);
    e2=e[i]*e[i];
    Y=1./(1.-e2);
    edot=-1./15.*mu/M/M*(pow(Z,8))*e[i]*(304.+121.*e2)*(pow(Y,2.5));
    nudot=96./(10.*M_PI)*mu/(pow(M,3))*(pow(Z,11))*
      ((1.+(73./24.)*e2+(37./96.)*(e2*e2))*(pow(Y,3.5))+
       Z*Z*(pow(Y,4.5))*(12.5*(1.+49./24.*e2-255./96.*e2*e2-37./96.*e2*e2*e2)
			 -1./5376.*(46832.+198664.*e2-15330*e2*e2-12753.*e2*e2*e2)));
    e[i+1]=e[i]-edot*timestep;
    nu[i+1]=nu[i]-nudot*timestep;
  }
...
/*     Integrates f from t=0 to t=tend using NC-4.1.14.
       This gives the integral with error O(1/vlength^4)
       f is a vector f(i), where i=0->vlength
       Actual time at step i is t=timestep*i  (sec) */
...
void waveform(double tend,double *par, double nu0, int vlength, double timestep, double *hI, double *hII, int nmodes, double zeta, int WCN) {
  int i,i0,n;
...
void GenBCWave(double *hI, double *hII, double deltat, int vlength, double e0, double nu0, double M, double mu, double S, double dist, double inc) {
  ...
  double delta,deltahalf,t0,tend,q[14],element;
  ...
  /* These are phase angles - hardcoded for the moment, but can change this at a later date. */
  //     gim0
  par[5]=0.;
  //     Phi0
  par[6]=0.;
  ...
  waveform(tend,par,nu0,vlength,deltat,hI,hII,nmodes,zeta,0);
  return;
}
...
int main(int argc, char **argv) {
  ...
  double dt=1.; /* Sampling rate */
  int vlength=86400; /* Number of data points required. */

  double nu0=0.000313576;
  double e0=0.3;
  double M=1.e6;
  double mu=10.;
  double S=0.8;
  ...
  hI=(double *)malloc(vlength*sizeof(double));
  hII=(double *)malloc(vlength*sizeof(double));

  GenBCWave(hI, hII, dt, vlength, e0, nu0, M, mu, S, dist, inc);

  int i,j;
  FILE *hfile;
  char filename[60];
    sprintf(filename,"BCWave.dat");
    hfile=fopen(filename,"w");
    double t=0.;
    for (i=0;i<vlength;i++) {
      fprintf(hfile,"%8.6e %14.12e %14.12e\n",t,hI[i],hII[i]);
      t+=dt;
    }
    fclose(hfile);
}
